...
class TestDayNightTemplates:
    ...
    def test_nighttime_settings_has_default_mode_bypass_permissions(self):
        """Nighttime settings should use defaultMode: bypassPermissions for unattended safety."""
        path = self._template_path("nighttime_settings.json")
        if not os.path.isfile(path):
            pytest.skip()
        with open(path, encoding="utf-8") as f:
        ...
        assert data.get("permissions", {}).get("defaultMode") == "bypassPermissions", (
            "nighttime_settings.json should have defaultMode: bypassPermissions"
        )
