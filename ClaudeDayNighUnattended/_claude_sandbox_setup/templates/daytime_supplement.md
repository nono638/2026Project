...
- For each `done` task (without `daytime_reviewed`): read `WrittenByNighttime/<task>/result.md`.
  ...
  Note the `branch` field — the night branch needs review before merging.
...
**2. Handle unreviewed night branches** (if any `done` tasks have branches not yet merged):
> "[task-001] branch `night/task-001-...` hasn't been merged yet.
> **A** *(recommended)* — Review the diff and merge now (run `git diff main...night/task-001-...`).
> **B** — Leave it, I'll handle it separately.
...
**When you promote:**
1. Write `DaytimeNighttimeHandOff/WrittenByDaytime/task-NNN-short-name/spec.md`
2. Write pre-written tests in `tests/`
3. Add entry to `tracker.json` with `"status": "todo"`
4. In `incubating.md`: mark `**Promoted to:** task-NNN` and move the entry to `archive/`
