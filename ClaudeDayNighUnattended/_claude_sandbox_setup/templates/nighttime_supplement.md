...
| State | What to do |
|-------|-----------|
| On `main`, clean working tree | Normal start — proceed to step 1 |
| On a `night/...` branch, clean working tree | Run `git log main..HEAD --oneline`. If commits exist: implementation is done — skip to bookkeeping (result.md, tracker update, move files, checkout main). If no commits: resume implementation from plan.md. |
| On a `night/...` branch, dirty working tree | Uncommitted changes from a crash. Run `git diff` to see what's there, then continue implementation from that point. Do not discard the changes. |
| On `main`, dirty working tree | Unexpected. Stash the changes (`git stash`) with a message, log what happened in nighttime.log, then proceed normally. |

1. Read `DaytimeNighttimeHandOff/tracker.json`
2. Check for any tasks with `"status": "in_progress"` — these were interrupted mid-run. Resume
   them first using the git state you already assessed above.
3. Skip any tasks with `"status": "blocked"` — these need human input. You cannot proceed on them.
...
- **Never install packages globally.** No `pip install` without an active venv. No
  `npm install -g`. All dependencies go into the project.
